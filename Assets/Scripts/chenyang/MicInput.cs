...
public class MicInput : MonoBehaviour {
	...
	void Update () {
        ...
        {
            objPrefab.gameObject.transform.localScale = Vector3.one*volume* scaleValme*5;

            Instantiate(objPrefab, objPos.position, Quaternion.identity);
            timer = 0;
            isCreatObj = false;

            Invoke("EarthQuake", 1.5f);
            time = 0;
        ...
        if (time > 5f)
        {  
            mainCam.GetComponent<PostEffectScript>().enabled = false;
            time = 0;
        }
    }
    ...
    {
        mainCam.GetComponent<PostEffectScript>().enabled = true;
    }
